...
echo "### dependencies ###"
apt-get update -qq
apt-get install -y -qq python3.7 python3-pip
#ln -s /usr/bin/python3 /usr/bin/python && ln -s /usr/bin/pip3 /usr/bin/pip
apt-get install -y -qq git vim
apt-get install -y -qq --no-install-recommends sudo bzip2 wget curl nano vim-tiny lsof htop net-tools
apt-get install -y -qq --no-install-recommends libcurl3 libssl1.0.0 zlib1g libuuid1
apt-get install -y -qq supervisor openjdk-8-jre 
...
echo "### download ###"
mkdir /tmp/blazing/
cd /tmp/blazing/ && wget -q https://s3.amazonaws.com/blazingsql-colab/blazingsql-files.tar.gz && tar -xf /tmp/blazing/blazingsql-files.tar.gz
